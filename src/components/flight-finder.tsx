"use client";

import { useState } from "react";
import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { Button } from "@/components/ui/button";
import { Loader2, ArrowRight, Clock, IndianRupee, Briefcase } from "lucide-react";
import Image from "next/image";
import { Separator } from "./ui/separator";

const mockFlightResults = [
  {
    airline: "IndiGo",
    number: "6E 202",
    from: "Mumbai (BOM)",
    to: "Delhi (DEL)",
    departure: "08:30",
    arrival: "10:45",
    duration: "2h 15m",
    price: "4500",
    luggage: "15kg",
    image: "https://placehold.co/600x400.png",
    imageHint: "airplane wing"
  },
  {
    airline: "Vistara",
    number: "UK 990",
    from: "Mumbai (BOM)",
    to: "Delhi (DEL)",
    departure: "11:00",
    arrival: "13:10",
    duration: "2h 10m",
    price: "5200",
    luggage: "15kg",
    image: "https://placehold.co/600x400.png",
    imageHint: "airport terminal"
  },
  {
    airline: "Air India",
    number: "AI 888",
    from: "Mumbai (BOM)",
    to: "Delhi (DEL)",
    departure: "14:20",
    arrival: "16:35",
    duration: "2h 15m",
    price: "4850",
    luggage: "25kg",
    image: "https://placehold.co/600x400.png",
    imageHint: "clouds sky"
  },
];

export function FlightFinder() {
  const [searching, setSearching] = useState(false);
  const [results, setResults] = useState<any[] | null>(null);

  const handleSearch = async () => {
    setSearching(true);
    setResults(null);
    await new Promise((resolve) => setTimeout(resolve, 1500));
    setResults(mockFlightResults);
    setSearching(false);
  };

  return (
    <div className="space-y-8">
      <Card>
        <CardHeader>
          <CardTitle>Search Flights</CardTitle>
          <CardDescription>
            Aggregate the best flight deals for your journey.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <Input placeholder="From Airport" />
            <Input placeholder="To Airport" />
            <Input type="date" />
          </div>
        </CardContent>
        <CardFooter>
          <Button onClick={handleSearch} disabled={searching} className="w-full sm:w-auto">
            {searching && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            {searching ? "Searching..." : "Search Flights"}
          </Button>
        </CardFooter>
      </Card>

       {searching && (
         <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-8 text-center">
            <Loader2 className="h-12 w-12 animate-spin text-primary" />
            <h3 className="mt-4 text-lg font-semibold">Finding Flights...</h3>
            <p className="mt-2 text-sm text-muted-foreground">Scanning the skies for the best deals.</p>
        </div>
      )}

      {results && (
        <div className="space-y-4">
          <h2 className="text-2xl font-bold">Available Flights</h2>
          {results.map((flight, index) => (
            <Card key={index} className="overflow-hidden transition-shadow hover:shadow-lg">
               <div className="grid grid-cols-1 md:grid-cols-3">
                 <div className="md:col-span-1">
                   <Image
                    src={flight.image}
                    alt={flight.airline}
                    width={600}
                    height={400}
                    data-ai-hint={flight.imageHint}
                    className="h-full w-full object-cover"
                  />
                </div>
                 <div className="md:col-span-2">
                  <CardHeader>
                    <CardTitle>{flight.airline} ({flight.number})</CardTitle>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        <span>{flight.from}</span>
                        <ArrowRight className="h-4 w-4" />
                        <span>{flight.to}</span>
                    </div>
                  </CardHeader>
                  <CardContent className="grid grid-cols-2 gap-4 md:grid-cols-3">
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-accent"/>
                      <div>
                        <p className="font-semibold">{flight.departure} - {flight.arrival}</p>
                        <p className="text-xs text-muted-foreground">{flight.duration}</p>
                      </div>
                    </div>
                     <div className="flex items-center gap-2">
                      <IndianRupee className="h-4 w-4 text-accent"/>
                      <div>
                        <p className="font-semibold">₹{flight.price}</p>
                        <p className="text-xs text-muted-foreground">per person</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <Briefcase className="h-4 w-4 text-accent"/>
                      <div>
                        <p className="font-semibold">Cabin</p>
                        <p className="text-xs text-muted-foreground">{flight.luggage}</p>
                      </div>
                    </div>
                  </CardContent>
                  <Separator className="my-2" />
                  <CardFooter>
                     <Button className="w-full sm:w-auto bg-accent text-accent-foreground hover:bg-accent/90">Book Now</Button>
                  </CardFooter>
                 </div>
               </div>
            </Card>
          ))}
        </div>
      )}
    </div>
  );
}
